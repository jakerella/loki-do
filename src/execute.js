...
var mod = {
	...
	showUsage: function () {
		console.log([
			...
			"                  NOTE: This command requires the 'subdomain' option and a cloud",
			"                        configuration block (currently only 'digitial_ocean')",
			"                  NOTE: 'provision' will also checkout your project code",
			"                        from source control, run 'npm provision', then run",
			"                        'npm install' (which will also run 'pre-' and 'postinstall",
			...
			"                  NOTE: because we use a 'safe' mechanism for running custom",
			"                        commands, you MUST include this step separately in the",
			"                        build process, it will not run automatically before 'start'",
